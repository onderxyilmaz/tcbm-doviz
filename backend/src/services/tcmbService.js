...
async function fetchTCMBData(series, startDate, endDate) {
  try {
    ...
    // Örnek: https://evds2.tcmb.gov.tr/service/evds/series=TP.DK.USD.S&startDate=01-01-2026&endDate=10-02-2026&type=json
    ...
    console.log(`TCMB API Request: ${url}`);
...
export async function getCurrentRates(currencies = ['USD', 'EUR', 'CHF']) {
  ...
  for (const currency of currencies) {
    ...
    try {
      ...
      // Response yapısını kontrol et ve logla
      console.log(`Buy Data structure for ${currency}:`, JSON.stringify(buyData).substring(0, 500));
      console.log(`Sell Data structure for ${currency}:`, JSON.stringify(sellData).substring(0, 500));

      // TCMB API response yapısı kontrolü
      // Response yapısı genellikle şu formatta olabilir:
      // { items: [{ Tarih: "...", value: "..." }] }
      // veya { items: [{ DATE: "...", TP_DK_USD_A: "..." }] }
      // veya direkt array: [{ Tarih: "...", value: "..." }]
      ...
      console.log(`Buy Items count for ${currency}:`, buyItems.length);
      console.log(`Sell Items count for ${currency}:`, sellItems.length);
      ...
      if (buyItems && buyItems.length > 0) {
        ...
        for (let i = buyItems.length - 1; i >= 0; i--) {
          ...
          // Farklı formatlar: value, TP_DK_USD_A, TP_DK_EUR_A, TP_DK_CHF_A, vs.
          let value = null;
          const seriesCode = CURRENCY_CODES[currency].buy; // Örn: TP.DK.USD.A
          const seriesKey = seriesCode.replace(/\./g, '_'); // TP_DK_USD_A
          
          // Önce genel value alanını kontrol et
          value = item?.value || item?.VALUE || item?.[seriesKey] || item?.[seriesCode];
          
          // Eğer hala bulunamadıysa, tüm anahtarları kontrol et
          if (!value && item) {
            const keys = Object.keys(item);
            // Series koduna benzer bir anahtar ara
          ...
          console.log(`Buy item ${i} for ${currency}:`, { value, tarih, itemKeys: Object.keys(item || {}) });
          
          if (value !== null && value !== undefined && value !== '' && !isNaN(parseFloat(value))) {
            buyRate = parseFloat(value);
            lastDate = tarih;
            console.log(`Found buy rate for ${currency}:`, buyRate, 'on date:', lastDate);
            break;
          }
        }
      }
      ...
      if (sellItems && sellItems.length > 0) {
        for (let i = sellItems.length - 1; i >= 0; i--) {
          ...
          // TCMB API response'unda value alanını bul
          let value = null;
          const seriesCode = CURRENCY_CODES[currency].sell; // Örn: TP.DK.USD.S
          const seriesKey = seriesCode.replace(/\./g, '_'); // TP_DK_USD_S
          ...
          console.log(`Sell item ${i} for ${currency}:`, { value, tarih, itemKeys: Object.keys(item || {}) });
          ...
          if (value !== null && value !== undefined && value !== '' && !isNaN(parseFloat(value))) {
            ...
            console.log(`Found sell rate for ${currency}:`, sellRate, 'on date:', lastDate);
            break;
          }
        }
      }
...
export async function getHistoricalRates(currency, startDate, endDate) {
  ...
  console.log(`Historical Buy Data structure for ${currency}:`, JSON.stringify(buyData).substring(0, 500));
  console.log(`Historical Sell Data structure for ${currency}:`, JSON.stringify(sellData).substring(0, 500));
  ...
  console.log(`Historical Buy Items count for ${currency}:`, buyItems.length);
  console.log(`Historical Sell Items count for ${currency}:`, sellItems.length);
  ...
  if (buyItems && buyItems.length > 0) {
    buyItems.forEach((item, index) => {
      // TCMB API response'unda value alanını bul
      let value = null;
      const seriesKey = buySeries.replace(/\./g, '_'); // TP_DK_USD_A
      ...
      if (index < 3) {
        console.log(`Historical Buy item ${index} for ${currency}:`, { value, date, itemKeys: Object.keys(item || {}) });
      }
  ...
  if (sellItems && sellItems.length > 0) {
    sellItems.forEach((item, index) => {
      // TCMB API response'unda value alanını bul
      let value = null;
      const seriesKey = sellSeries.replace(/\./g, '_'); // TP_DK_USD_S
      ...
      if (index < 3) {
        console.log(`Historical Sell item ${index} for ${currency}:`, { value, date, itemKeys: Object.keys(item || {}) });
      }
  ...
  console.log(`Buy Map size for ${currency}:`, buyMap.size);
  console.log(`Sell Map size for ${currency}:`, sellMap.size);

  // Tüm tarihleri birleştir (hem buy hem sell items'lerden)
  const allDates = new Set();
  
  // Buy items'lerden tarihleri al
  ...
  // Sell items'lerden tarihleri al
  ...
  console.log(`Total rates found for ${currency}:`, rates.length);
